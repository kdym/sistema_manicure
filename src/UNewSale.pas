...
type
  TFrmNewSale = class(TForm)
    ...
    Label13: TLabel;
    LbFinalValue: TLabel;
    BtDeletePaymentMethods: TBitBtn;
    Label12: TLabel;
    LbChange: TLabel;
    ...
    procedure GrdPaymentMethodsCellClick(Column: TColumn);
    procedure BtDeletePaymentMethodsClick(Sender: TObject);
  private
...
procedure TFrmNewSale.BtDeletePaymentMethodsClick(Sender: TObject);
begin
  with QryPaymentMethods do
  begin
    Delete;
    ...
    if IsEmpty then
    begin
      BtDeletePaymentMethods.Enabled := false;
    end;
  end;
end;
...
procedure TFrmNewSale.GrdPaymentMethodsCellClick(Column: TColumn);
begin
  if not QryPaymentMethods.IsEmpty then
  begin
    BtDeletePaymentMethods.Enabled := true;
  end;
end;
...
var
  SumPayments, SumMoney, Change, Discount, TotalSale, Total: Double;
begin
  with QryPaymentMethods do
  begin
    SumPayments := 0;
    ...
    begin
      ...
      if FieldByName('tipo').AsString = TPaymentMethod.MONEY then
      begin
        SumMoney := SumMoney + FieldByName('valor').AsFloat;
      end;
  ...
  TotalSale := UValidator.ParseNumber(LbTotal.Caption);
  ...
  begin
    ...
    begin
      Discount :=
        (TotalSale * StrToFloat(UValidator.UnmaskNumber
  ...
  if SumMoney > TotalSale - Discount then
  begin
    Change := SumMoney - TotalSale + Discount;
  end;

  LbChange.Caption := Format('%m', [Change]);

  if Change <> 0 then
  begin
    Total := SumPayments - Change;
  end
  else
  begin
